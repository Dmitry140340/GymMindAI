...
function isFitnessQuestion(text) {
  ...
  const interfaceButtons = [
    ...
    // Кнопки помощи
    '💬 Как пользоваться ботом?', '⚡ Что умеет ИИ-тренер?', '💎 Информация о подписке',
...
async function handleTextMessage(bot, msg) {
  ...
  try {
    ...
    if (text === '💎 Информация о подписке' || text.includes('Информация о подписке')) {
      await bot.sendMessage(
        chatId,
        '💎 **Информация о подписке**\n\n' +
        '🆓 **Бесплатный доступ:**\n' +
        '• 7 запросов к ИИ-тренеру\n' +
        ...
        '⭐ **Стандартная подписка (599₽):**\n' +
        '• 300 запросов к ИИ-тренеру\n' +
        '• Все возможности месячной\n' +
        '• Приоритетная поддержка\n\n' +
        '🚀 **Премиум подписка (999₽):**\n' +
        '• 600 запросов к ИИ-тренеру\n' +
        '• Безлимитные ИИ-инструменты\n' +
        '• Эксклюзивные возможности\n' +
        '• VIP поддержка\n\n' +
        '🔄 Подписка автоматически продлевается каждый месяц.\n' +
        '💳 Для оформления перейдите в раздел "💎 Подписка"',
        { parse_mode: 'Markdown', ...helpKeyboard }
      );
      return;
    }
    ...
    if (['Жим лежа', 'Приседания', 'Становая тяга', 'Жим стоя', 'Подтягивания', 'Отжимания', 'Жим ногами', 'Тяга штанги', 'Сгибания на бицепс', 'Французский жим'].includes(text)) {
